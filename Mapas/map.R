...
cbind(table(map_comunas$idcomuna))

tm_shape(map_comunas)+
  tm_borders(col='red') 
...
names(data)
head(map[,c("NOMBRE","CODIGO")])
tail(map[,c("NOMBRE","CODIGO")], 10)
...
ggplot(map_comunas_merge) +
  ...
  geom_sf(data = map_metro, fill = "grey40", aes(color = "Líneal de metro"), size=.4) +
