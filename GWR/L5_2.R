...
map <- map |> 
  mutate(inc.b = gwr.fit2$SDF$inc)

inc.b <- ggplot(map) + 
  geom_sf(aes(fill = inc.b), color = "gray95", size = 0.05) +
  scale_fill_viridis_c(direction = -1) +
...
# T estadísticos y pvalor
map <- map |> 
  mutate(inc.t = gwr.fit2$SDF$inc/gwr.fit2$SDF$inc_se,
...
inc.pval <- ggplot(map) + 
  geom_sf(aes(fill = sig), color = "gray50", size = 0.05) +
  scale_fill_manual(values = c("#c61a09", "#ff6242", "#ffc9bb", "white"),
                    labels = c("P<0.01", "P<0.05", "P<0.1", "No significante")) +
...
ggview(units = "in", width = 8, height = 4, dpi = 300, bg="white")
...
inc.pvalms <- ggplot(mgwr_sf) + 
  geom_sf(aes(fill = sig), color = "gray50", size = 0.05) +
  scale_fill_manual(values = c("#c61a09", "#ff6242", "white"),
                    labels = c("P<0.01", "P<0.05", "P<0.1", "No significante")) +
  labs(fill="Significancia estadística") +
  theme_void() +
  theme(legend.position = c(.2,.5),
