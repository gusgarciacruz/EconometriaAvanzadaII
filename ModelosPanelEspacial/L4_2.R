...
# Anílisis de la tasa de muertes violentas (mur) para 1999
data_mur99 <- data |> 
...
moran.map[(moran.map$st_lmur >= 0 & 
           ...
           (moran.map$"Pr.z....E.Ii.." <= 0.05), "quad_sig"] <- "high-high"

# low-low quadrant
moran.map[(moran.map$st_lmur <= 0 & 
             moran.map$lag_st_lmur <= 0) & 
            (moran.map$"Pr.z....E.Ii.." <= 0.05), "quad_sig"] <- "low-low"

# high-low quadrant
moran.map[(moran.map$st_lmur >= 0 & 
             moran.map$lag_st_lmur <= 0) & 
            (moran.map$"Pr.z....E.Ii.." <= 0.05), "quad_sig"] <- "high-low"

# low-high quadrant
moran.map[(moran.map$st_lmur <= 0 & 
             moran.map$lag_st_lmur >= 0) & 
            (moran.map$"Pr.z....E.Ii.." <= 0.05), "quad_sig"] <- "low-high"

# non-significant observations
moran.map[(moran.map$"Pr.z....E.Ii.." > 0.05), "quad_sig"] <- "not signif."  
